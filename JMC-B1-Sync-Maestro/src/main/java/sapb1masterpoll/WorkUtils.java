...
public class WorkUtils {
		...
		if (!which.equals("Orders"))
		{
			...
			for (HashMap<String, Object> map : payload) {
				...
				for (String destination : possibleMaps.keySet()) {
					...
					//Contador de Lineas
					int i = 1;
					// Loopeo por cada linea para arreglar la numeración
					for (HashMap<String, Object> lineCurrentMap : documentLines) {
						...
						if (lineCurrentMap.get("LineTaxJurisdictions").getClass().getName().contains("List")) {
							...
							for (HashMap<String, Object> jurisdictionMapLine : (ArrayList<HashMap<String, Object>>) lineCurrentMap.get("LineTaxJurisdictions")) {
	...
	// Recursive function to remove all fields that are null > they would be ignored by SAP anyway
	@SuppressWarnings("unchecked")
